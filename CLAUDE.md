...
No topo da tela de Unidade/Departamento, mesma identidade do Controle de Fechamentos: **Total de SPEDs / Entregue / Pendente** — só 3 (`renderizarPlacares()`). `Entregue + Pendente = Total` (vêm das 2 abas do export). "Pendente" (→ `Status="Pendente"`) é clicável e filtra a tabela; "Total" e "Entregue" não. Entregue mostra "% do total"; Pendente mostra **"X já vencida(s)"** na descrição (subconjunto `Status="Pendente"` **e** `Atrasado="Atrasado"`).

**Não tem placar de "Atrasado" separado (decisão do usuário, 2026-08-27)**: chegou a ter (Total/Entregue/Pendente/Atrasado), mas o usuário achou que não ajudava — atraso cruza os outros dois (a grande maioria dos "Atrasado" já foi entregue, só que fora do prazo), então um card "Atrasado: 4.145" ao lado dava impressão errada de 4 mil problemas em aberto. O número acionável é só "pendente e já vencido" (~352), que virou a descrição do card Pendente. O **filtro** de Atraso (`#f-atraso`/`#t-atraso`) continua existindo.
