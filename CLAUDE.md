...
No topo da tela de Unidade/Departamento, mesma identidade do Controle de Fechamentos: **Total de SPEDs / Entregue / Pendente / Vencidas** (`renderizarPlacares()`). `Entregue + Pendente = Total` (as 2 abas do export). Clicáveis (filtram a tabela): **Pendente** (→ `Status="Pendente"`) e **Vencidas** (→ `Status="Pendente"` **+** `Atrasado="Atrasado"`, via `filtrarPorVariosEMostrarTabela`). Descrições: Entregue/Pendente = "% do total"; Vencidas = "% das pendentes".

**"Vencidas" ≠ total de atrasados (decisão do usuário, 2026-08-27)**: chegou a ter um card "Atrasado" (4.145), mas o usuário achou que não ajudava — atraso cruza Entregue/Pendente (a grande maioria dos "Atrasado" já foi entregue, só que fora do prazo), então "Atrasado: 4.145" ao lado dos outros dava impressão errada de 4 mil problemas em aberto. Trocado por **"Vencidas"** = só o subconjunto acionável: pendente **e** já passou do vencimento (~352 em SP/ICMS). O **filtro** de Atraso (`#f-atraso`/`#t-atraso`) continua existindo pra quem quiser o corte completo.
